   ...
   **/
   ...
   class CameraPointInfo {
    public:
      ...
      // 새로운 메서드들 (UI 파라미터 포함 - campt용)
      PvlGroup *SetImage(const UserInterface &ui, const double sample, const double line,
                         const bool outside = false, const bool error = false);
      PvlGroup *SetCenter(const UserInterface &ui, const bool outside = false, const bool error = false);
      PvlGroup *SetSample(const UserInterface &ui, const double sample, const bool outside = false,
                          const bool error = false);
      PvlGroup *SetLine(const UserInterface &ui, const double line, const bool outside = false,
                        const bool error = false);
      PvlGroup *SetGround(const UserInterface &ui, const double latitude, const double longitude,
                          const bool outside = false, const bool error = false);
    ...
    private:
      bool CheckCube();
      virtual PvlGroup *GetPointInfo(bool passed, bool outside, bool errors);
      PvlGroup *GetPointInfoWithOptions(const UserInterface &ui, bool passed, bool outside, bool errors);
